...
using RealTimeCharts.Microservices.ClientDispatcher;
...
namespace RealTimeCharts.Microservices.DataProvider.Test
{
    public class WorkerTest
    {
        ...
        public void ShouldStartToConsumeAfterSubscribedToEvent_WhenExecuted()
        {
            ...
            _eventBusStrict.InSequence(sequence).Setup(eb => eb.SubscribeTo<DataGeneratedEvent>());
            _eventBusStrict.InSequence(sequence).Setup(eb => eb.SubscribeTo<DataGenerationFinishedEvent>());
            _eventBusStrict.InSequence(sequence).Setup(eb => eb.StartConsuming());

            _sutStrict.StartAsync(default);

            _eventBusStrict.Verify(eb => eb.SubscribeTo<DataGeneratedEvent>());
            _eventBusStrict.Verify(eb => eb.SubscribeTo<DataGenerationFinishedEvent>());
